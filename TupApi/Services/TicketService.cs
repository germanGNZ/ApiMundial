...
namespace TupApi.Services
{
    public class TicketService
    {
        ...
        {
            if (!int.TryParse(ticket.PartidoId, out int numeroPartido))
                throw new ArgumentException("El PartidoId debe ser el número de partido (1-104).");

            var partido = await _partidoRepo.GetByNumeroAsync(numeroPartido);
