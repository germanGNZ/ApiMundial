...
namespace TupApi.Services
{
    public class PartidoService
    {
        ...
        public async Task PatchAsync(string id, PartidoPatchDto dto)
        {
            var existe = await _repo.GetByIdAsync(id);
            if (existe == null)
                throw new KeyNotFoundException($"No se encontró el partido con ID '{id}'.");

            if (dto.Definido == true)
            {
                var equipo1Final = dto.Equipo1?.Trim() ?? existe.Equipo1;
                var equipo2Final = dto.Equipo2?.Trim() ?? existe.Equipo2;

            if (equipo1Final.StartsWith("Gan.") || equipo1Final.StartsWith("Per."))
                throw new InvalidOperationException(
                    "No se puede marcar como definido: Equipo1 aún no está confirmado.");

            if (equipo2Final.StartsWith("Gan.") || equipo2Final.StartsWith("Per."))
                throw new InvalidOperationException(
                    "No se puede marcar como definido: Equipo2 aún no está confirmado.");
            }

            if (dto.Equipo1 != null) dto.Equipo1 = dto.Equipo1.Trim();
            if (dto.Equipo2 != null) dto.Equipo2 = dto.Equipo2.Trim();

            var actualizado = await _repo.PatchAsync(id, dto);
            if (!actualizado)
                throw new InvalidOperationException("No se pudo actualizar el partido.");
        }
