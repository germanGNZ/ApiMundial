...
namespace TupApi.Repositories{
    public class PartidoRepository : IPartidoRepository
    {
        ...
        public PartidoRepository(IMongoClient mongoClient, IOptions<MongoDbSettings> settings)
        {
            ...
            var indexKeys = Builders<Partido>.IndexKeys;
            _col.Indexes.CreateMany(new[]
            {
                new CreateIndexModel<Partido>(
                    indexKeys.Ascending(p => p.NumeroPartido),
                    new CreateIndexOptions { Unique = true }),
                new CreateIndexModel<Partido>(
                    indexKeys.Ascending(p => p.Fase)),
                new CreateIndexModel<Partido>(
                    indexKeys.Ascending(p => p.Grupo)),
            });
        }
        ...
        public async Task<bool> PatchAsync(string id, PartidoPatchDto dto)
        {
            var updates = new List<UpdateDefinition<Partido>>();
            var upd = Builders<Partido>.Update;
